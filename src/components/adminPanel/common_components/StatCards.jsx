...
const StatCards = ({ name, icon: Icon, value, color }) => {
    return (
      <motion.div
        className='bg-gray-900 bg-opacity-60 backdrop-blur-md shadow-lg rounded-lg border border-gray-800'
        whileHover={{ y: -6, boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.5)" }}
      >
        <div className='px-6 py-5 sm:p-7'>
          <span className='flex items-center text-sm font-medium text-gray-300'>
            <Icon size={22} className="mr-2" style={{ color }} />
            {name}
          </span>
          <p className='mt-2 text-gray-100 font-semibold text-2xl'>{value}</p>
        </div>
      </motion.div>
    );
  };
